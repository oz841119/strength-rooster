...
export default function Page() {
  ...
  return (
    <div>
      <DCard isFull title="RECORD" className="flex flex-wrap gap-3 justify-between">
        ...
        <DSelect className="w-48/100" color="default" items={annotation} label="註記標籤" onChange={(name) => onSelectChanege('tags', name)}></DSelect>
        <DSelect className="w-48/100" color="warning" items={menu} label="選擇菜單" onChange={(name) => onSelectChanege("menu", name)}></DSelect>
        <DSelect className="w-48/100 mb-6" color="secondary" items={action} label="訓練動作" onChange={(name) => onSelectChanege('action', name)}></DSelect>
